...
export function placeLabels(data, size, padding) {
  ...
  data.forEach(function(d) {
    ...
    if (d.labelPlaced) {
      ...
      minTextHeight =
        d.textHeight < minTextHeight ? d.textHeight : minTextHeight;
    }
  });
  ...
  data.forEach(function(d) {
    ...
    if (d.labelPlaced) {
      findAvailablePosition(d, bitMaps, padding, function() {
        ...
        return  !checkCollision(d.extendedSearchBound, bitMaps.mark) &&
...
function findAvailablePosition(datum, bitMaps, padding, checkAvailability) {
  ...
  for (i = datum.currentPosition[0]; i <= 1 && !datum.labelPlaced; i++) {
    for (j = initJ; j <= 1 && !datum.labelPlaced; j++) {
      ...
      if (
        searchBound.startX < 0 ||
        searchBound.startY < 0 ||
        searchBound.endY >= bitMaps.mark.height ||
        searchBound.endX >= bitMaps.mark.width
      ) {
        continue;
      }
