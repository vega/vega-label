...
export function BitMap(_width, _height) {
  this.pixelSize = Math.sqrt(_width * _height / 1000000.0);
  ...
  this.unmarkInBoundBinned = function (x, y, x2, y2) {
    ...
    for (; y <= y2; y++) {
      ...
      indexEnd = end >>> DIV;
      if (indexStart === indexEnd) {
        this.array[indexStart] &= right1[start & MOD] | right0[(end & MOD) + 1];
      } else {
        this.array[indexStart] &= right1[start & MOD];
        this.array[indexEnd] &= right0[(end & MOD) + 1];
