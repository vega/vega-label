  ...
  if (marktype === "group") {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (group.marktype === "area") {
        ...
        if (
          placeLabel(d, layer1, layer2, anchors, offsets, allowOutside, context)
        ) {
          d.opacity = d.originalOpacity;
        }
      } else {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (x = x1; x <= x2; x += pixelSize) {
      for (y = y1; y <= y2; y += pixelSize) {
        ...
        hi = height;
        if (
          !checkCollisionFromPositionAndHeight(
            textWidth,
            textHeight,
            x,
            y,
...
function checkCollisionFromPositionAndHeight(
  textWidth,
  textHeight,
  x,
  y,
...
function placeLabel(
  ...
  for (i = 0; i < n; i++) {
    ...
    yc =
      markBound[4 + dy] +
      (isIn * textHeight * dy) / 2.0 +
      offsets[i] * dy * sizeFactor;
    ...
    if (allowOutside) {
      ...
      searchBoundY1 =
        searchBoundY1 < 0 ? 0 : searchBoundY1 > h - 1 ? h - 1 : searchBoundY1;
      searchBoundY2 =
        searchBoundY2 < 0 ? 0 : searchBoundY2 > h - 1 ? h - 1 : searchBoundY2;
    }

    if (!textWidth) {
      // var end = searchX + (searchBoundY2 - searchBoundY1) * (~~(text.length / 3));
      if (
        layer1.searchOutOfBound(searchX, searchBoundY1, searchX, searchBoundY2)
      )
        continue;
      if (
    ...
    if (allowOutside) {
      searchBoundX1 =
        searchBoundX1 < 0 ? 0 : searchBoundX1 > w - 1 ? w - 1 : searchBoundX1;
      searchBoundX2 =
        searchBoundX2 < 0 ? 0 : searchBoundX2 > w - 1 ? w - 1 : searchBoundX2;
    }

    if (
      layer1.searchOutOfBound(
        searchBoundX1,
        searchBoundY1,
        searchBoundX2,
        searchBoundY2
