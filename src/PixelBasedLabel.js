...
export default function placeLabels(
  ...
  var context = canvas().getContext('2d'),
    grouptype = marktype === 'group' && data[0].datum.datum.items[markIdx].marktype,
    height = size[1],
    width = size[0];
  var d, mb, hidden;
  ...
  if (grouptype === 'area') {
    for (i = 0; i < n; i++) {
      ...
      if (!hidden && placeLabelInArea(d, height, avoidBaseMark, layer1, layer2, context))
...
function placeLabelInArea(d, height, avoidBaseMark, layer1, layer2, context) {
...
function placeLabel(d, layer1, layer2, anchors, offsets, context) {
